...
const setupGame = () => {
  player = new Player(p5.width / 2, p5.height / 2, 0, 0, 6, 0.08, 0.5, 0.6, 40, 12, p5.color(255, 255, 255));
  ...
  if (game === true) {
    ...
    if (dead === false) {
      ...
      if (press === true) {
        if (p5.millis() > timeB) {
          timeB = p5.millis() + 200;
          let xpos = player.x + p5.cos(player.angle + p5.PI / 2.0) * player.d;
          let ypos = player.y + p5.sin(player.angle + p5.PI / 2.0) * player.d;
          bullets.add(new Bullet(xpos, ypos, 12, player.angle, 7, p5.color(255, 255, 255)));
        }
      }
      if (p5.millis() > timeA) {
        timeA = p5.millis() + 200;
        let angle = p5.random(0.0, 2.0 * p5.PI);
        let xpos = p5.width / 2.0 + p5.cos(angle) * p5.sqrt(p5.sq(p5.height) + p5.sq(p5.width)) / 2 + p5.random(50.0, 150.0);
        let ypos = p5.height / 2.0 + p5.sin(angle) * p5.sqrt(p5.sq(p5.height) + p5.sq(p5.width)) / 2 + p5.random(50.0, 150.0);
        let randomS = p5.random(1.0, 4.0);
        let randomA = p5.random(-90.0, 90.0);
        asteroids.add(new Asteroid(xpos, ypos, (-1) * randomS * p5.cos(angle + randomA), p5.random(-3.0, 3.0) + (-1) * randomS * p5.sin(angle + randomA), p5.random(70, 200), p5.color(180)));
      }
    }
    if (dead === true) {
      p5.textSize(60);
...
class Bullet {
  constructor(Tx, Ty, speed, angle, Td, Tc) {
    this.x = 0;
    this.y = 0;
    this.xs = 0;
    this.ys = 0;
    ...
    this.x = Tx;
    this.y = Ty;
    this.xs = speed * p5.cos(angle + p5.PI / 2);
    this.ys = speed * p5.sin(angle + p5.PI / 2);
    this.d = Td;
    this.c = Tc;
  }
  move() {
